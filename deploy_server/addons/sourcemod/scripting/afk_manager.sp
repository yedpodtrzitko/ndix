...
new Handle:hCvarWFPTime =					INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	else if (strcmp(game_mod, "tf", false) == 0)
	{
		...
		hCvarWFPTime = FindConVar("mp_waitingforplayers_time");
	}
	...
	{
		ND = true;
		hCvarWFPTime = FindConVar("sm_warmup_time");
	}
/*
...
CheckWFP()
{
	if (hCvarWFPTime != INVALID_HANDLE)
	{
		// warmup is not enabled
		if (ND && !GetConVarBool(FindConVar("sm_nd_balancer_enable"))) {
			return;
		}

		if ((GetConVarFloat(hCvarWFPTime) - 1.0) > 0.0)
		{
#if _DEBUG
			...
			CreateTimer(GetConVarFloat(hCvarWFPTime), WFPEnd);
		}
	}
}

public Action:WFPEnd(Handle:timer) {
	#if _DEBUG
		LogDebug(false, "WFP is over");
	#endif
	bWaitRound = false;
	return Plugin_Continue;
}
...
public Action:Event_PlayerTeamPost(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//TODO this can be un-intended, dont want misleading diff atm
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// round has started, dont need any mumbo-jumbo checking
	bWaitRound = false;
