...
public Action:Command_StartMutiny(client, args)
{
	...
	new iTeam = GetClientTeam(client);
	if (iTeam < 2 || g_iCommanders[iTeam - 2] == -1)
	{
		return Plugin_Handled;
	}

	// it was commander, so... resign?
	if (client == g_iCommanders[iTeam - 2])
	{
	...
	if (!g_bMutinyVoteAllowed[iTeam - 2]) {

		PrintToChat(client, "\x05[SM] %T", "Not Enough Time", LANG_SERVER);
		return Plugin_Handled;
	}

	if (GetNumPlayersOnTeam(iTeam) < g_iMutinyVoteMinPlayers) {
		return Plugin_Handled;
	}

	new PlayerManager = FindEntityByClassname(-1, "nd_player_manager")
	new level = GetEntProp(PlayerManager, Prop_Send, "m_iPlayerRank", 1, client);

	if (level < 15) {
		PrintToChat(client, "[SM] Mutiny cant be started by player with lvl < 15");
		return Plugin_Handled;
	}
