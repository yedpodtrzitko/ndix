...
public Action:Command_JoinTeam(client, String:command[], argc) 
{
	...
	{
		PrintToChat(client,"This is an event, please stay in Spectate until you are choosen.");
		return Plugin_Handled;
	}
}
...
public Action:PlayerTeam(client,args) 
{
	...
	decl String:player_nam[64]
	decl String:team_str[64]
	GetCmdArg(1, player_nam, sizeof(player_nam));
	GetCmdArg(2, team_str, sizeof(team_str));
	new team_num=StringToInt(team_str);
	new target;
	if (StrContains(player_nam, "STEAM_1:0:") != -1) 
	{
		StripQuotes(player_nam);
		TrimString(player_nam);
		target = -1;
	} else 
	{
		target = FindTarget(client, player_nam, false, false);
		comm_con=target;
		if (target == -1) return Plugin_Handled;
		GetClientAuthString(target, player_nam, sizeof(player_nam));
	}
